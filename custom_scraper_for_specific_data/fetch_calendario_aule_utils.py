...
def clean_html_tags(text):
    if not text:
        return ""
    # 1. Remove all HTML tags (e.g., <i ...> or </i>)
    # This regex matches anything between < and >
    clean_text = re.sub(r'<[^>]+>', '', text)
    
    # 2. Convert HTML entities like &ograve; to real characters (ò)
...
def get_rooms(text, site):
    ...
    # extract only rooms for the requested site
    if site not in rooms_list:
        raise ValueError(f"Site '{site}' not found")

    site_rooms = rooms_list[site]
    return site_rooms
...
def get_data(site, start_date, end_date, TEMP_DIR):
    ...
    for day in days:
        ...
        if response_data is None:
            print(f"Failed to retrieve data for {site['label']} on {day}. URL: {build_units_url(payload, 'https://orari.units.it/agendaweb/index.php')}")
            continue
        if isinstance(response_data["events"], list) and not response_data["events"]:
            continue
        filtered_json = response_filter(response_data)
        # with open("response.json", "w", encoding="utf-8") as f:
        #     json.dump(filtered_json, f, ensure_ascii=False, indent=2)

        final_json.extend(filtered_json)

    write_json_to_file(final_json, TEMP_DIR, site['label'], start_date, end_date)
